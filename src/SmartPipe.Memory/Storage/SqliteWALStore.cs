...
public sealed class SqliteWALStore : IGraphStore
{
    ...
    public async Task<Node> UpsertNodeAsync(Node node, CancellationToken ct = default)
    {
        ...
        try
        {
            ...
            cmd.Parameters.AddWithValue(
                "@validFrom",
                node.ValidFrom.ToUniversalTime().ToString("O")
            );
            cmd.Parameters.AddWithValue(
                "@validTo",
                node.ValidTo?.ToUniversalTime().ToString("O") ?? (object)DBNull.Value
            );
    ...
    /// <inheritdoc />
    public async Task BatchUpsertNodesAsync(
    ...
    private async Task InsertBatchAsync(List<Node> batch, CancellationToken ct)
    {
        ...
        for (var i = 0; i < batch.Count; i++)
        {
            var node = batch[i];
            values.Add(
                $"(@id{i}, @type{i}, @label{i}, @props{i}, @health{i}, @fail{i}, @pred{i}, @strain{i}, @vfrom{i}, @vto{i}, @txtime{i}, @ver{i})"
            );
            ...
            cmd.Parameters.AddWithValue(
                $"@vto{i}",
                node.ValidTo?.ToString("O") ?? (object)DBNull.Value
            );
    ...
    public async Task<Edge> UpsertEdgeAsync(Edge edge, CancellationToken ct = default)
    {
        ...
        try
        {
            ...
            cmd.Parameters.AddWithValue(
                "@validTo",
                edge.ValidTo?.ToString("O") ?? (object)DBNull.Value
            );
            cmd.Parameters.AddWithValue("@txTime", DateTime.UtcNow.ToString("O"));
    ...
    /// <inheritdoc />
    public IAsyncEnumerable<Node> QueryNodesAsync(
        MemoryQuery query,
        CancellationToken ct = default
    ) => _memoryStore.QueryNodesAsync(query, ct);

    /// <inheritdoc />
    public IAsyncEnumerable<Node> QueryNodesAsOfAsync(
        MemoryQuery query,
        DateTime asOf,
        CancellationToken ct = default
    ...
    /// <inheritdoc />
    public IAsyncEnumerable<Edge> QueryEdgesAsOfAsync(
        MemoryQuery query,
        DateTime asOf,
        CancellationToken ct = default
    ...
    /// <inheritdoc />
    public IAsyncEnumerable<Edge> QueryInsightsAsync(
        MemoryQuery query,
        CancellationToken ct = default
    ...
    public Task UpdateNodeHealthAsync(
        ...
        _memoryStore.UpdateNodeHealthAsync(
            nodeId,
            healthScore,
            failureProb,
            predictedLatencyMs,
    ...
    private static Node ReadNode(SqliteDataReader reader)
    {
        return new Node
        {
            ...
            ValidTo = reader.IsDBNull(9)
                ? null
                : DateTimeOffset.Parse(reader.GetString(9)).UtcDateTime,
    ...
    private static Edge ReadEdge(SqliteDataReader reader)
    {
        return new Edge
        {
            ...
            ValidTo = reader.IsDBNull(8)
                ? null
                : DateTimeOffset.Parse(reader.GetString(8)).UtcDateTime,
